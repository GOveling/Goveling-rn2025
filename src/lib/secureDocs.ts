...
export function encryptBytes(bytes: Uint8Array, password: string){
  ...
  const cipher = CryptoJS.AES.encrypt(wordArray, key, { iv: ivWord, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 });
  ...
  return { ciphertext_b64: ct, iv_b64: ivb64, hmac_b64: hmac, algo: 'AES-256-CBC+HMAC-SHA256', salt };
}

export function decryptBytes(ciphertext_b64: string, iv_b64: string, hmac_b64: string, password: string, salt: string){
  const key = deriveKey(password, salt);
  ...
  const cipherParams = CryptoJS.lib.CipherParams.create({ ciphertext: CryptoJS.enc.Base64.parse(ciphertext_b64) });
  ...
  for (let i=0;i<decrypted.sigBytes;i++){ arr[i] = (decrypted.words[(i/4)|0] >> (24 - 8*(i%4))) & 0xff; }
  return arr;
}

// Upload encrypted file to Supabase Storage and register metadata
export async function uploadSecureDoc(user_id:string, title:string, bytes:Uint8Array, password:string){
  const enc = encryptBytes(bytes, password);
  const path = `secure_docs/${user_id}/${Date.now()}-${title.replace(/\s+/g,'_')}.bin`;
  const blob = new Blob([Buffer.from(enc.ciphertext_b64, 'base64')], { type: 'application/octet-stream' });
  const up = await supabase.storage.from('private').upload(path, blob, { upsert: true, contentType: 'application/octet-stream' });
  if (up.error) throw up.error;
  await supabase.from('secure_documents').insert({
    user_id, title, storage_path: path, size_bytes: bytes.length, iv: enc.iv_b64, hmac: enc.hmac_b64, algo: enc.algo
  });
  return path;
}
...
export async function enqueueSecureDoc(user_id:string, title:string, bytes:Uint8Array, password:string){
  const enc = encryptBytes(bytes, password);
  const item = { user_id, title, ciphertext_b64: enc.ciphertext_b64, iv_b64: enc.iv_b64, hmac_b64: enc.hmac_b64, algo: enc.algo, created_at: Date.now() };
...
export async function flushQueue(){
  ...
  let ok=0; const remain=[];
  for (const it of arr){
    try{
      const path = `secure_docs/${it.user_id}/${Date.now()}-${it.title.replace(/\s+/g,'_')}.bin`;
      const blob = new Blob([Buffer.from(it.ciphertext_b64, 'base64')], { type: 'application/octet-stream' });
      const up = await supabase.storage.from('private').upload(path, blob, { upsert: true, contentType: 'application/octet-stream' });
      if (up.error) throw up.error;
      await supabase.from('secure_documents').insert({
        user_id: it.user_id, title: it.title, storage_path: path, size_bytes: Math.round(it.ciphertext_b64.length*3/4), iv: it.iv_b64, hmac: it.hmac_b64, algo: it.algo
      });
      ok++;
