...
type FlightQuery = { from:string; to:string; depart:string; return?:string; pax?:number; cabin?:'eco'|'prem'|'bus'|'first' };
type HotelQuery  = { city:string; checkin:string; checkout:string; guests?:number; rooms?:number };
...
export const affiliates = {
  flights: {
    ...
    buildUrl(q:FlightQuery){
      ...
      const base = process.env.EXPO_PUBLIC_FLIGHTS_AFFIL_URL || 'https://example-flights.com/search';
      ...
      if (process.env.EXPO_PUBLIC_FLIGHTS_AFFIL_ID) u.searchParams.set('affid', process.env.EXPO_PUBLIC_FLIGHTS_AFFIL_ID);
      const url = u.toString(); return url;
    },
    async open(q:FlightQuery){ const url = this.buildUrl(q); await logClickout('flights', url, q); return open(url); }
  },
  hotels: {
    enabled: true,
    buildUrl(q:HotelQuery){
      ...
      if (process.env.EXPO_PUBLIC_HOTELS_AFFIL_ID) u.searchParams.set('affid', process.env.EXPO_PUBLIC_HOTELS_AFFIL_ID);
      const url = u.toString(); return url;
    },
    async open(q:HotelQuery){ const url = this.buildUrl(q); await logClickout('hotels', url, q); return open(url); }
  },
  esim: {
    enabled: true,
    buildUrl(q:EsimQuery){
      ...
      if (process.env.EXPO_PUBLIC_ESIM_AFFIL_ID) u.searchParams.set('affid', process.env.EXPO_PUBLIC_ESIM_AFFIL_ID);
      const url = u.toString(); return url;
    },
    async open(q:EsimQuery){ const url = this.buildUrl(q); await logClickout('esim', url, q); return open(url); }
  }
};
