...
const getUserTripIds = async (userId: string): Promise<string[]> => {
  ...
  const { data: collabTrips } = await supabase.from('trip_collaborators').select('trip_id').eq('user_id', userId);
...
export function useFavorites() {
  ...
  const loadFavorites = useCallback(async () => {
    try {
      ...
      const { data: collabTrips } = await supabase.from('trip_collaborators').select('trip_id').eq('user_id', user.id);
