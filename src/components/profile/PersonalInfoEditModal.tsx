...
import { View, Text, Platform, Modal, ScrollView, TextInput, TouchableOpacity, ActivityIndicator, KeyboardAvoidingView, Alert, FlatList } from 'react-native';
...
export const PersonalInfoEditModal: React.FC<Props> = ({ visible, onClose, userId, userEmail, onSaved }) => {
  ...
  const CityItem = React.memo(({ item, isSelected, onPress }: {
    ...
    >
      ...
      <View style={{ flex: 1 }}>
        <Text style={[styles.optionText, isSelected && styles.optionTextSelected]}>{item.city}</Text>
        {item.population > 0 && (
                        ...
                        if (displayedCitiesCount < cities.length && !citySearchQuery) {
                          return (
                            <View style={{ padding: 16, alignItems: 'center' }}>
                              <TouchableOpacity
                                style={{ backgroundColor: '#F3F4F6', paddingHorizontal: 16, paddingVertical: 8, borderRadius: 6, borderWidth: 1, borderColor: '#E5E7EB' }}
