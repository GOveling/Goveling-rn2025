...
serve(async (_req)=>{
  ...
  for (const q of queue || []){
    // tokens for user
    const { data: tokens } = await supabase.from("device_tokens").select("*").eq("user_id", q.user_id);
  ...
  return new Response(JSON.stringify({ ok:true, processed: (queue||[]).length }), { headers:{ "Content-Type":"application/json" } });
});
