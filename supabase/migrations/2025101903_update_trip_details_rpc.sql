-- Allows owners and editors to update trip details safely under RLS via SECURITY DEFINER
-- This function enforces that only:
--  - the trip owner (user_id or owner_id matches auth.uid())
--  - or an active collaborator with role = 'editor'
-- can update selected fields of a trip.
...
is 'SECURITY DEFINER function to update trip details for owners and editors, bypassing RLS safely.';
