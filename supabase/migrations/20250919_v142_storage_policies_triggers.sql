...
-- Enable RLS on storage.objects and add path-based policies for avatars/docs
begin;
alter table if exists storage.objects enable row level security;

-- Public bucket: read for all, write only own avatars
create policy if not exists "Public read" on storage.objects
  for select using ( bucket_id = 'public' );

create policy if not exists "Avatar write own" on storage.objects
...
create policy if not exists "Avatar update own" on storage.objects
  for update using (
    bucket_id = 'public'
    and (storage.foldername(name))[1] = 'avatars'
    and (storage.foldername(name))[2] = auth.uid()::text
...
create policy if not exists "Avatar delete own" on storage.objects
  for delete using (
    bucket_id = 'public'
    and (storage.foldername(name))[1] = 'avatars'
    and (storage.foldername(name))[2] = auth.uid()::text
...
-- Private bucket: only owner can read/write 'docs/{uid}/*'
create policy if not exists "Docs read own" on storage.objects
  for select using (
    bucket_id = 'private'
...
create policy if not exists "Docs write own" on storage.objects
  for insert with check (
    bucket_id = 'private'
    and (storage.foldername(name))[1] = 'docs'
    and (storage.foldername(name))[2] = auth.uid()::text
...
create policy if not exists "Docs update own" on storage.objects
  for update using (
    bucket_id = 'private'
    and (storage.foldername(name))[1] = 'docs'
    and (storage.foldername(name))[2] = auth.uid()::text
...
create policy if not exists "Docs delete own" on storage.objects
  for delete using (
    bucket_id = 'private'
    and (storage.foldername(name))[1] = 'docs'
    and (storage.foldername(name))[2] = auth.uid()::text
