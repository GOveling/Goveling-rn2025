...
export default function SecureDocs(){
  ...
  return (
    <View style={{ flex:1, padding:12, gap:8 }}>
      ...
      <TouchableOpacity onPress={add} style={{ backgroundColor:'#007aff', padding:12, borderRadius:8, marginTop:8 }}>
        <Text style={{ color:'#fff', textAlign:'center', fontWeight:'600' }}>Subir documento</Text>
      </TouchableOpacity>
      
      <TouchableOpacity 
        onPress={async()=>{ const n = await flushQueue(); if (n) load(); }} 
        style={{ borderWidth:1, borderColor:'#eee', padding:10, borderRadius:8 }}
      >
        <Text>Sincronizar pendientes</Text>
      </TouchableOpacity>
      ...
      <FlatList
        ...
        ListEmptyComponent={<Text style={{ opacity:0.6, marginTop:10 }}>No hay documentos aún</Text>}
      />
    </View>
  );
}
